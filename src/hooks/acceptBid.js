...
export const AcceptBid = () => {
    ...
    const AcceptBidHook = useCallback(
        async (collectionAddr, tokenID) => {
            ...
            try {
                ...
                const gas = await contract.methods.acceptBid(collectionAddr, tokenID).estimateGas({ from: account });
                const staked = await contract.methods.acceptBid(collectionAddr, tokenID).send({ from: account, gasPrice: gasFunPrice , gas: gas })
    ...
    return { AcceptBidHook: AcceptBidHook };
};
