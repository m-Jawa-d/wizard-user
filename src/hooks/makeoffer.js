    ...
    const Offers = useCallback(
        async (adder, tokenID, fee) => {
            ...
            try {
                ...
                const gas = await contract.methods.sendOffer(adder, tokenID, weiAmount).estimateGas({ from: account, gasPrice: gasFunPrice })
                const staked = await contract.methods.sendOffer(adder, tokenID, weiAmount).send({from: account, gas: gas, gasPrice: gasFunPrice })
