...
const AuthorActivity = () => {
    ...
    const getNftActivityDetails = async () => {
        ...
        config = {
            method: "get",
            url: `${api_url}/users/activities?offset=1&limit=10&orderField=createdAt&orderDirection=-1`,
            headers: {
        ...
        axios(config)
            .then(function (response) {
                setdataset3(response?.data?.data?.userActivites)
                                    ...
                                    (
                                        <div className="single-item" >
                                            ...
                                            <div className="right-content">
                                                ...
                                                <h6>
                                                    <span className='text-span-color'> Transferred From  {' '}
                                                        {item?.from?.slice(0, 8)}...
                                                        {item?.from?.slice(
                                                            item?.from?.length - 6,
                                                            item?.from?.length
                                        ...
                                        (
                                            <div className="single-item" key={item.id}>
                                                ...
                                                <div className="right-content">
                                                    <h5>{item?.launchpadId?.name} #{item?.tokenId}</h5>
                                                    <h6>
                                                        <span className='text-span-color'> Bid By {' '}
                                                            {item?.from?.slice(0, 8)}...
                                                            {item?.from?.slice(
                                                                item?.from?.length - 6,
                                                                item?.from?.length
                                                            )}
                                                            {' '} For Price {item?.price} {' '} Core
                                                        </span>
                                                    </h6>
                                                    <p>{moment(item?.createdAt).fromNow()}</p>
                                                </div>
                                        ...
                                        (
                                            <div className="single-item" key={item.id}>
                                                ...
                                                <div className="right-content">
                                                    <h5>{item?.launchpadId?.name} #{item?.tokenId}</h5>
                                                    <h6>
                                                        <span className='text-span-color'> Listed By {' '}
                                                            {item?.from?.slice(0, 8)}...
                                                            {item?.from?.slice(
                                                                item?.from?.length - 6,
                                                                item?.from?.length
